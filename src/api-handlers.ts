...
export function handleHome(
	...
	const home =
		allowedPaths.length > 0 ? path.resolve(allowedPaths[0]) : os.homedir();
...
export async function handleLs(
	...
	try {
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to list: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleRead(
	...
	try {
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to read: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleWrite(
	...
	try {
		...
		console.log(
			`[telegram-files] WRITE ${filePath} by token ${tokenTag(req)}`,
		);
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to write: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleUpload(
	...
	if (
		...
		jsonResponse(
			res,
			400,
			{ error: "dir and valid name required" },
			corsOrigin,
		);
		return;
	}
	...
	try {
		...
		console.log(
			`[telegram-files] UPLOAD ${filePath} (${size} bytes) by token ${tokenTag(req)}`,
		);
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to upload: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleMkdir(
	...
	try {
		...
		console.log(
			`[telegram-files] MKDIR ${dirPath} by token ${tokenTag(req)}`,
		);
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to mkdir: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleDelete(
	...
	if (await isAllowedRoot(targetPath, allowedPaths)) {
		jsonResponse(
			res,
			403,
			{ error: "cannot delete a root allowed path" },
			corsOrigin,
		);
		return;
	}
	...
	try {
		...
		console.log(
			`[telegram-files] DELETE ${targetPath} by token ${tokenTag(req)}`,
		);
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to delete: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
...
export async function handleSearch(
	...
	if (query.length < 1 || query.length > 256) {
		jsonResponse(
			res,
			400,
			{ error: "query parameter 'q' required (1-256 chars)" },
			corsOrigin,
		);
		return;
	}
	...
	try {
		...
		jsonResponse(
			res,
			500,
			{ error: `Failed to search: ${sanitizeError(err)}` },
			corsOrigin,
		);
	}
}
